...
columns_list = ["Names","Price","P/E ratio", "PEG ratio", "Market Cap (B)","Analysts Review","52-Week Range","Sector"]
...
for i in range(0, df2.shape[1]):  # command to know the dtypes
    if i >= limit:
        pass
    else:
...
df2['PEG ratio'] = pd.to_numeric(df2['PEG ratio'])
df2 = df2.loc[df2['PEG ratio']<50]  # Selects only good PEG ratio stocks (good growth)
df2 = df2.loc[df2['Analysts Review']!="Sell"]  # It removes stocks with "sell" from the Analysts Review
df2 = df2.loc[df2['Analysts Review']!="Underperform"]  # It removes stocks with "underperform" from the Analysts Review
